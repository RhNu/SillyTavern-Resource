import { IMGGEN_BLOCK_STATE_UPDATED_EVENT, REGEX_NAME } from '@/ImgGenHelper/app/ids';
...
import { buildFinalImagePrompt } from '@/ImgGenHelper/features/image-generation/prompt-builder';
import {
  buildAutoGenerationQueueToastMessage,
  hasAutoGenerationQueueWork,
  type AutoGenerationQueueSnapshot,
} from '@/ImgGenHelper/features/image-generation/queue-policy';
import { buildImgGenRefFilterRegexString } from '@/ImgGenHelper/features/image-generation/ref-codec';
import { requestImageGeneration } from '@/ImgGenHelper/features/image-generation/request';
...
} from '@/ImgGenHelper/features/message-cards/controller';
import type { TaskResultToast } from '@/ImgGenHelper/features/tasking/task-events';
import type { TaskProjection as TaskCenter, TaskGroupFocus } from '@/ImgGenHelper/features/tasking/task-projection';
import { logError, logWarn } from '@/ImgGenHelper/shared/log';
import { showInfoToast, showWarningToast } from '@/ImgGenHelper/shared/toast';
...
function buildAutomaticQueueCompletionSummary(session: AutomaticQueueSession): AutomaticQueueCompletionSummary {
  ...
  return {
    ...
    message: processedCount > 0 ? `自动生图队列结束，${parts.join('，')}` : '自动生图队列结束，没有可处理的图片任务',
  };
}
