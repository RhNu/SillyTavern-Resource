...
2. 根据生图模型选择指导模板：V4.5 使用简洁 Danbooru 标签串，V5 使用简短自然语言、标签式描述或两者混合。两者都将构图、基本动作与场景放在主提示词，并让逐角色提示词只描述角色本身。
3. 先按正文规则选择正文，再执行清洗；清洗后的每个非空行生成 A1、A2 等候选锚点，模型通过 `anchor_id`
   选点。图片记录统一写入聊天变量，再向正文提交稳定图片引用。
4. 把图片块交给内部工作流队列，逐个执行 `validate → generate-and-store → commit → associate`
   四个阶段。服务端插件把图片直接写入当前用户的 SillyTavern 图片目录，前端只接收路径描述符。
5. 使用后端返回的图片路径渲染图片卡片，并将新输出单向登记到 SillyTavern 的
...
- 阶段化重试：只重跑失败的阶段。每次逻辑生成使用稳定的
  `operationId`；响应丢失或后端存储暂时失败时，插件会复用已经生成好的图片，不会再次消耗 NovelAI 配额。
- 登记失败时，自动重试和之后的手动重试都只补写 `chat_backgrounds`，不会重新生成或追加输出。
...
URL（仅 Custom 使用）、模型 ID，以及生成与存储超时、自动重试次数与请求间隔；真实 API
...
- 设置 schema v10 会从 v9 保留现有配置并移除已经不需要的浏览器上传超时；更早的旧版正文清洗字段不会迁移。
...
`imggen-novelai` v2 同时支持 `stored` 与 `binary` 输出。当前脚本固定使用
`stored`，并沿用原生上传接口的角色目录和文件名清洗规则；需要直接获取图片字节的其他调用方可显式选择
`binary`。本脚本不会在删除图片块或聊天时删除已保存文件。
