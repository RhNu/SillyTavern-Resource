...
3. 保留短对白，按原文的安全块边界生成 A1、A2 等候选锚点；模型通过 `anchor_id`
...
v8 保存 Provider ID、凭证 ID、Custom Base
...
- 空行分隔普通正文；全文没有空行时按单换行划分。列表、表格、引用和 HTML 包裹内部不插入图片；未闭合的包裹保留为参考正文但不提供内部锚点。
- 提取规则按原文顺序排列并去重。包裹内的多个片段共享一个安全边界，清洗不会产生原文中不存在的插入位置。
- 已移除最小段落长度设置，短句不再被过滤。v7 设置升级到 v8 时只移除该字段，保留清洗规则、连接、模板、角色配置和生图参数。
- 自定义模板中的旧段落协议不会被自动改写；检测到旧引用时以 toast 提示改用 `anchor_id`。
