...
function choosePrimaryKey(
  entry: SelectiveTargetEntry,
  primaryKeys: string[],
): {
  key: string;
  ...
  const safePrimaryKey = primaryKeys.find(
    key => !wouldTriggerNegatedSecondary(key, secondaryStringKeys, secondary.logic),
  );
  if (safePrimaryKey) {
    return {
      key: safePrimaryKey,
      warningReason:
        secondaryStringKeys.length < secondary.keys.length ? 'negated_secondary_contains_regex_key' : undefined,
    };
  }
...
export async function collectSelectiveEntries(
  worldbookNames: string[],
  logger: Logger,
): Promise<SelectiveTargetEntry[]> {
