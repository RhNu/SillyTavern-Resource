...
- `util/st`: SillyTavern 原生能力和 UI 桥接
- `util/tavern-helper`: 酒馆助手脚本宿主、生命周期等生态工具
- `util/react`: React 挂载适配和共享组件
- `util/common.ts`: 尚未拆分的通用函数
- `util/mvu.ts`: MVU 角色卡工具
...
`util/tavern-helper/lifecycle/chat.ts` 中定义好的工具函数:
