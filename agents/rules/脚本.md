...
`util/tavern-helper/dom/styles.ts` 中的 `teleportStyle` 函数来将样式复制到酒馆网页的 `<head>` 中.
...
`util/tavern-helper/dom/script-host.ts` 中的 `createScriptIdIframe` 函数来创建 iframe DOM, 并将 vue 组件在 iframe load 完成后挂载到 iframe 内部的 body 上 (`iframe_element.contentDocument!.body`):
